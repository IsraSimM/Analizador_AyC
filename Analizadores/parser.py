...
def p_declaration(p):
    '''declaration : TIPODATO IDENTIFICADOR PUNTOYCOMA
                   | TIPODATO IDENTIFICADOR CORCHETEIZQ NUMERO CORCHETEDER PUNTOYCOMA'''
    p[0] = ('declaration', p[1], p[2]) if len(p) == 4 else ('array_declaration', p[1], p[2], p[4])
...
def p_expression(p):
    ...
    p[0] = p[1] if len(p) == 2 else p[2] if len(p) == 4 and p[1] == '(' else (p[2], p[1], p[3])
...
def p_for_statement(p):
    '''for_statement : FOR LPAREN assignment_statement PUNTOYCOMA condition PUNTOYCOMA assignment_statement RPAREN block'''
